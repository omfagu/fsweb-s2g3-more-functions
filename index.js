...
function ortalamaBul(dizi) {
  if (dizi.length !== 0){let toplam = dizi.reduce((oncekiDeger,suankiDeger) => oncekiDeger+suankiDeger);
    let sonuc = toplam/dizi.length ;
    return  Math.ceil(sonuc);}
    else {return null;}
  
}
let sayiDizisi = [109, 216, 288, 143, 71, 185, -278, 194, 5]
console.log("output: " + ortalamaBul(sayiDizisi))
...
function ortalamadanBuyukleriBul(dizi, cbortalamaBul) {
  
  if (dizi.length != 0) {let ort = cbortalamaBul(dizi);
    let sonuc = [];
...
console.log(ortalamadanBuyukleriBul(sayiDizisi ,ortalamaBul));
